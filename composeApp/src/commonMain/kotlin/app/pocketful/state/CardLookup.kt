...
class CardLookup(private val api: TcgDex) {
    ...
    /**
     * Network failures reach the user as one sentence about what to do, not as a stack
     * of Ktor exception names. The distinction that matters on a phone is "you are
     * offline" versus "the catalog is down", and neither is worth a class name.
     */
    private fun offlineMessage(cause: Throwable): String =
        "Could not reach the card catalog. Check your connection and try again."
            .takeIf { cause.message?.contains("resolve", ignoreCase = true) == true ||
                cause.message?.contains("connect", ignoreCase = true) == true }
            ?: "The card catalog did not answer. Try again in a moment."
}
