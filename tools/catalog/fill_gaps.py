...
Three tiers, best first, because no single source covers them all:
  ...
  2. TCGplayer product  A product photo, keyed by the id TCGdex hands out in
                        ...
                        pull_catalog.py --prices captures those ids into prices/.
...
# and much harder to notice. So candidates are suggested by --suggest and adopted here
# only once a person has looked at them.
SET_ALIASES = {
...
def tcgplayer_ids(set_id: str) -> dict[str, list[int]]:
    """Product ids captured by `pull_catalog.py --prices`, keyed by card id."""
    path = CATALOG / "prices" / f"{set_id}.json"
    if not path.exists():
        return {}
    doc = json.loads(path.read_text(encoding="utf-8"))
    return {c["id"]: c["tcgplayer"] for c in doc.get("cards", []) if c.get("tcgplayer")}
