...
KINDS = {
    ...
    "short-set": "fewer cards than the set claims to hold",
...
def inspect(set_id: str, doc: dict) -> tuple[Counter, dict[str, list[str]]]:
    ...
    # A set that says it holds 102 cards and lists 94 is missing eight, and no
    # per-card check can see that -- the eight are not there to be checked.
    official = (doc.get("cardCount") or {}).get("official") or 0
    if official and len(cards) < official:
        counts["short-set"] += official - len(cards)
        offenders["short-set"].append(f"{len(cards)} of {official}")
